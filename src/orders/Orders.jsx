  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const status = record.fullfilledDate || record.fullfiledDate ? 'Completed' : 'Pending';
      ...
      render: (_, record) => (
        <Space>
          <Button
            type="text"
            icon={<EyeOutlined />}
            onClick={() => message.info(`View order ${record.id} - Coming soon!`)}
            style={{ color: '#14b8a6' }}
          />
          <Button
            type="text"
    ...
    <div className="min-h-screen bg-zinc-900 w-screen p-6">
      ...
      >
        <Spin spinning={loading} tip="Creating order...">
          ...
          >
            <Item
              label={<span style={{ color: '#18181b' }}>Quantity Machines</span>}
